...
- `services/slack_authorization.py` - `is_authorized_slack_user`: the `SLACK_BAN_AUTHORIZED_USERS` allowlist check for the ban menu, fail-closed on unset/empty (#152).
- `routers/slack_interactivity.py` - `POST /slack/interactivity`: the Slack app's single interactivity Request URL. Handles the ban menu's `block_actions` (open modal) and `view_submission` (authorize, ban, ack, edit) (#152).
