...
`services/slack.maybe_append_ban_button` appends a "Ban requester" actions block to a post's blocks iff its fingerprint normalizes to a usable UUID -- the same usable/unusable split as `build_slack_metadata`, so the button and the metadata it depends on can never disagree. The button carries no `value`; clicking it fires a `block_actions` payload at `POST /slack/interactivity` (`routers/slack_interactivity.py`), which reads the fingerprint back out of the *clicked message's own metadata*, not from anything the client set. That handler:
