...
- `generated/api_models.py` - Pydantic v2 models generated from `wxyc-shared/api.yaml`. Do not edit by hand; regenerate via `bash scripts/generate_api_models.sh`. The script prefers a sibling `wxyc-shared/` directory and falls back to downloading `api.yaml` from GitHub. That download authenticates when `GH_TOKEN`/`GITHUB_TOKEN` is set — anonymous raw.githubusercontent.com requests share a per-IP budget across the Actions runner pool and intermittently 429 ([#268](https://github.com/WXYC/request-o-matic/issues/268)) — and falls back to an anonymous retry, so local runs need no token. The script's header comment holds the rationale and the token-safety properties; `tests/unit/test_generate_api_models_auth.py` pins them.
...
- `routers/slack_interactivity.py` - `POST /slack/interactivity`: the Slack app's single interactivity Request URL. Channel-agnostic by construction -- it reads `channel` from the payload and never compares it to `SLACK_CHANNEL_ID`, so it serves a menu posted in any channel the bot is in. Dispatches the ban menu's `block_actions` (open modal) and both `view_submission` callbacks -- the ban modal (authorize, ban, ack, edit) (#152) and the moderator roster save (#240).
