...
### In-Slack ban button ([#152](https://github.com/WXYC/request-o-matic/issues/152))
`services/slack.maybe_append_ban_button` appends a "Ban requester" actions block to a post's blocks iff its fingerprint normalizes to a usable UUID -- the same usable/unusable split as `build_slack_metadata`, so the two can never disagree about whether a fingerprint was *given*. Whether the button actually has anything to act on is a separate question: `build_slack_metadata`'s envelope is a `chat.postMessage`-only parameter, silently dropped by the incoming-webhook transport, so a webhook-posted message's button renders but finds no fingerprint on click. `routers/slack_interactivity.py` posts an ephemeral explaining this rather than no-op'ing silently; see the caveat in [`docs/admin-bans.md`](admin-bans.md). The button carries no `value`; clicking it fires a `block_actions` payload at `POST /slack/interactivity`, which reads the fingerprint back out of the *clicked message's own metadata*, not from anything the client set. That handler:
...
2. On a button click, opens a reason modal via `views.open`, stashing the fingerprint, channel, message ts, and (size permitting -- Slack caps `private_metadata` at 3000 characters) the message's own blocks minus the button, in the modal's `private_metadata`.
3. On submit (`view_submission`), re-authorizes the acting user against `SLACK_BAN_AUTHORIZED_USERS` server-side (`services/slack_authorization.is_authorized_slack_user`; unset/empty allowlist denies everyone), validates the reason is 1-1000 characters (matching `BanCreateRequest`), then calls `services/ban_service.ban(fingerprint, reason, actor=None)` -- the identical function `routers/admin.py` calls, so a Slack click and a curl produce the same audit trail. `actor` is always `None` from rom: Backend-Service's `banned_by_user_id` foreign-keys to better-auth's `user.id`, which a Slack user has no row in, so the Slack actor is recorded in Slack itself (the ephemeral ack + the edited "banned by @dj" footer) rather than in that column.
...
- `services/slack_authorization.py` - `is_authorized_slack_user`: the `SLACK_BAN_AUTHORIZED_USERS` allowlist check for the ban button, fail-closed on unset/empty (#152).
- `routers/slack_interactivity.py` - `POST /slack/interactivity`: the Slack app's single interactivity Request URL. Handles the ban button's `block_actions` (open modal) and `view_submission` (authorize, ban, ack, edit) (#152).
