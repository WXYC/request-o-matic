...
Two files are watched by keyword rather than wholesale (`KEYWORD_WATCHED`). `core/dependencies.py` also builds the LML, PostHog, and ban clients, and `config/settings.py` carries Slack/LML/PostHog/ban config; over the 120 days before this was written they changed on 10 days, of which exactly one touched a Groq line. Watching them whole would have spent the shared TPM bucket on ban-roster and LML work about half the nights the gate fired. They now trigger only when the diff's own `+`/`-` lines mention `groq`.
