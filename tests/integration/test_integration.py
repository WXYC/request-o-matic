...
class TestParserIntegration:
    ...
    async def test_bare_artist_name_parsed_as_artist(self):
        ...
        assert result.artist is not None, "Should extract artist name"
        assert (
            "lenderman" in result.artist.lower()
    ...
    async def test_greeting_not_parsed_as_song_title(self):
        ...
        assert result.artist is not None, "Should extract artist name"
        assert (
            "fleetwood" in result.artist.lower()
    ...
    async def test_parses_song_title_with_well_prefix(self):
        ...
        assert result.is_request is True, "Should recognize as a request"
        assert result.artist is not None, "Should extract artist"
        assert "monk" in result.artist.lower(), f"Expected Thelonious Monk, got: {result.artist}"
        assert result.song is not None, "Should extract song title 'Well, You Needn't'"
        assert (
            "needn" in result.song.lower()
    ...
    async def test_parses_editorial_commentary_as_album_request(self):
        ...
        assert (
            "stereopathic" in result.album.lower()
