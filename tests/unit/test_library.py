...
class TestFallbackSearch:
    ...
    async def test_fallback_removes_articles(self, test_db: LibraryDB):
        ...
        results = await test_db.search(query="The Stone Roses The Stone Roses", limit=10)
    ...
    async def test_fallback_with_punctuation(self, test_db: LibraryDB):
        ...
        results = await test_db.search(
            query="Richard D. James Album = リチャード・D. ジェイムス・アルブム", limit=10
        )
...
class TestFilteredSearch:
    ...
    async def test_search_by_artist_and_title_filter(self, test_db: LibraryDB):
        ...
        results = await test_db.search(artist="Stone Roses", title="Second Coming", limit=10)
...
class TestEdgeCases:
    ...
    async def test_unicode_characters(self, test_db: LibraryDB):
        ...
        results = await test_db.search(query="Aphex Twin リチャード 中文 العربية", limit=10)
...
class TestRealWorldScenarios:
    ...
    async def test_goon_gumpas_scenario(self, test_db: LibraryDB):
        ...
        results = await test_db.search(query=f"Aphex Twin {discogs_album_name}", limit=5)
