...
- **[`docs/admin-bans.md`](docs/admin-bans.md)** — Operator runbook for `/admin/bans` (request-line ban management), the in-Slack "Ban requester" menu (`POST /slack/interactivity`, #152 -- built and deployed, but **detached from the public request post** because Slack has no per-viewer block visibility; awaiting a moderators-channel entry point), and the `/request-mods` moderator roster (`POST /slack/commands`, #240): endpoints, curl examples, status codes, where to find a fingerprint, who can ban and what happens when the roster upstream is down. Both the HTTP admin router (#151) and the Slack-native router (#152) share `services/ban_service.py`. Ban authorization is the union of `SLACK_BAN_AUTHORIZED_USERS` (break-glass) and the Backend-Service roster, failing closed to the former.
- **[`docs/ban-system.html`](docs/ban-system.html)** — Standalone explainer for the whole request-line ban system, written for station operators as much as engineers: what the fingerprint is and why the device is the ban target, where ban state lives, the two-list authorization union and its fail-closed direction, why the ban menu is **not** on public request posts (Slack has no per-viewer block visibility), the operator procedure, and the system's known limits. Open it from a checkout — self-contained, no external resources, not hosted. Commands and endpoints deliberately live only in `docs/admin-bans.md`.
